...
class StreamProcessor(object):
    ...
    def __init__(self, midi_obj, slap_freq_map):
        self.spectral_analyser = SpectralAnalyser(
            window_size=WINDOW_SIZE,
            segments_buf=RING_BUFFER_SIZE)
        self.midi = midi_obj
        self.slap_freq_map = slap_freq_map
    ...
    def process_frame(self, data, frame_count, time_info, status_flag):
        ...
        if freq0:
            # slap is detected
            print "Slap detected; fundamental frequency: %s" %(freq0)            
            ...
            for key in self.slap_freq_map.keys():
                ...
                if ((freq0 > slap_freq["min_freq"]) and 
                    ...
                    for note in notes:
                        self.send_note("note_on", note)
                        self.send_note("note_off", note)

        return (data, paContinue)

    def send_note(self, cmd, note):
        midi_msg = self.midi.convert_midi_msg({
                        "cmd": cmd, "note": note['note'],
...
def main(com_port, midi_port, baud_rate, freq_config):
    ...
    # slap frequency maps initiation
    current_map = 0
    slap_freq_maps = slap_freqs(freq_config)
